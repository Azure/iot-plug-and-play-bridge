...
bool ModbusTcp_CloseDevice(SOCKET socket, HANDLE hLock)
{
	...
	while (0 < byteRcvd)
	{
		byteRcvd = recv(socket, buffer, MODBUS_RESPONSE_MAX_LENGTH, 0);
	}
...
int ModbusTcp_SendRequest(SOCKET socket, byte *requestArr, DWORD arrLen)
{
	...
	int totalBytesSent = send(socket, requestArr, arrLen, 0);
...
int ModbusTcp_ReadResponse(SOCKET socket, byte *response, DWORD arrLen)
{
	...
	bytesReceived = recv(socket, response + totalBytesReceived, arrLen - totalBytesReceived, 0);
