...
#define MODBUS_ADAPTER_NAME ModbusPnp
#define MODBUS_PROP_HANDLER_ADAPTER_METHOD ModbusPnp_PropertyUpdateHandlerRedirect
#define MODBUS_CMD_HANDLER_ADAPTER_METHOD ModbusPnp_CommandUpdateHandlerRedirect

#include "Adapters/ModbusPnp/CmdPropHandlers.h"
...
int ModbusPnp_OpenDevice(ModbusDeviceConfig* deviceConfig, JSON_Object* interfaceConfig, PNPBRIDGE_NOTIFY_DEVICE_CHANGE DeviceChangeCallback) {
	...
	deviceContext->ModbusDeviceChangeCallback = DeviceChangeCallback;
...
int ModbusPnp_StartDiscovery(PNPBRIDGE_NOTIFY_DEVICE_CHANGE DeviceChangeCallback, const char* deviceArgs, const char* adapterArgs) {
	if (deviceArgs == NULL) {
		return -1;
	}
	...
	ModbusPnp_OpenDevice(deviceConfig, interfaceConfigObj, DeviceChangeCallback);
...
int ModbusPnp_CreatePnpInterface(PNPADAPTER_CONTEXT adapterHandle, PNP_DEVICE_CLIENT_HANDLE pnpDeviceClientHandle, PPNPBRIDGE_DEVICE_CHANGE_PAYLOAD args) {
	
	PMODBUS_DEVICE_CONTEXT deviceContext = (PMODBUS_DEVICE_CONTEXT)args->Context;
	...
	while (interfaceDefHandle != NULL) {
		...
		PNP_CLIENT_READWRITE_PROPERTY_UPDATED_CALLBACK_TABLE modbusPropertyTable = { 0 };
		char** commandNames = NULL;
		int commandCount = 0;
		PNP_COMMAND_EXECUTE_CALLBACK* commandUpdateTable = NULL;
		PNP_CLIENT_COMMAND_CALLBACK_TABLE modbusCommandTable = { 0 };
		const ModbusInterfaceConfig* interfaceConfig = singlylinkedlist_item_get_value(interfaceDefHandle);
		...
		// Construct property table
		{
			...
			if (readWritePropertyCount > 0)
			{
				...
				propertyUpdateTable = calloc(1, sizeof(PNP_READWRITE_PROPERTY_UPDATE_CALLBACK*)*readWritePropertyCount);
				...
				for (int i = 0; i < readWritePropertyCount; i++)
				{
					...
					propertyUpdateTable[i] = MdobusPropertyHandlerTables[i];
				}
		...
		// Construct command table
		{
			...
			if (commandCount > 0)
			{
				...
				commandUpdateTable = calloc(1, sizeof(PNP_COMMAND_EXECUTE_CALLBACK*)*commandCount);
				...
				for (int i = 0; i < commandCount; i++)
				{
					...
					if (NULL != commandItemHandle) {
						...
						commandUpdateTable[i] = ModbusCommandHandlerTables[i];
					}
				}
		...
		pnpInterfaceClient = PnP_InterfaceClient_Create(pnpDeviceClientHandle, interfaceId,
			propertyCount > 0 ? &modbusPropertyTable : NULL,
			commandCount > 0 ? &modbusCommandTable : NULL,
			deviceContext);
