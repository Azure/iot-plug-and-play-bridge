    ...
    int Av)
{
    ...
    case R700_STATUS_NO_CONTENT:
        ...
        jsonVal_Rest = ImpinjReader_RequestGet(PnpComponentHandle, device, RestRequest->Requst, NULL, &httpStatus);
    ...
    size_t *CommandResponseSize)
{
    ...
    for (i = 0; i < (sizeof(R700_REST_LIST) / sizeof(IMPINJ_R700_REST)); i++)
    {
        ...
        {
            switch (R700_REST_LIST[i].Requst)
            {
            ...
            case PROFILES_START:
            {
                ...
                {
                    if (R700_REST_LIST[i].Requst == PROFILES_INVENTORY_PRESETS_ID_SET)
                    {
            ...
            {
            case GET:
                jsonVal_Rest = ImpinjReader_RequestGet(PnpComponentHandle, device, R700_REST_LIST[i].Requst, restParameter, &httpStatus);
                break;
            case PUT:
                jsonVal_Rest = ImpinjReader_RequestPut(PnpComponentHandle, device, R700_REST_LIST[i].Requst, restParameter, restBody, &httpStatus);
                break;
            case POST:
                jsonVal_Rest = ImpinjReader_RequestPost(PnpComponentHandle, device, R700_REST_LIST[i].Requst, restParameter, &httpStatus);
                break;
            case DELETE:
                jsonVal_Rest = ImpinjReader_RequestDelete(PnpComponentHandle, device, R700_REST_LIST[i].Requst, restParameter, &httpStatus);
                break;
            }
    ...
    if (r700_api != UNSUPPORTED)
    {
        ...
        case SYSTEM_POWER_SET:
        {
            ...
            if ((jsonVal_Power = ImpinjReader_RequestGet(PnpComponentHandle, device, R700_REST_LIST[SYSTEM_POWER].Requst, NULL, &httpStatusGetPower)) != NULL)
            {
