...
#define STREAM_DATA_BUFFER_SIZE 100000
  ...
  sprintf(fullStreamUrl, "%s%s", session_data->basePath, "/data/stream");
...
curlStaticGet(
    ...
    int *statusCode)
{
  LogInfo("R700 : curlStaticGet %s", endpoint);
  ...
  LogInfo("R700 : CurlCode %d HttpStatus %ld",
          res,
          http_code);
    ...
    int *statusCode)
{
  ...
  LogInfo("R700 : curlStaticPost %s", endpoint);
  ...
  LogInfo("R700 : POST Endpoint: %s", fullurl);
    ...
    int *statusCode)
{
  ...
  LogInfo("R700 :R700 : curlStaticDelete %s", endpoint);
