...
set build-winsdkver=10.0.17763.0
...
if "%1" equ "--winsdkver" goto arg-win-sdk-ver
call :usage && exit /b 1
...
if %build-platform% == x64 (
    set CMAKE_DIR=pnpbridge_x64
) else if %build-platform% == arm (
    set CMAKE_DIR=pnpbridge_arm
) else if %build-platform% == x86 (
    set CMAKE_DIR=pnpbridge_x86
) else if %build-platform% == arm64 (
    set CMAKE_DIR=pnpbridge_arm64
)
goto args-continue
...
if %MAKE_NUGET_PKG% == yes (
    ...
    if EXIST %cmake-root%\cmake\pnpbridge_x64 (
        rmdir /s/q %cmake-root%\cmake\pnpbridge_x64
    )
    mkdir %cmake-root%\cmake\pnpbridge_x64
    rem no error checking

    pushd %cmake-root%\cmake\pnpbridge_x64
    cmake -Drun_longhaul_tests:BOOL=%CMAKE_run_longhaul_tests% -Drun_e2e_tests:BOOL=%CMAKE_run_e2e_tests% -Drun_sfc_tests:BOOL=%CMAKE_run_sfc_tests% -Drun_unittests:BOOL=%CMAKE_run_unittests% %build-root% -Duse_prov_client:BOOL=%prov_auth% -G "Visual Studio 15 Win64" -Duse_edge_modules=%use_edge_modules%
    if not !ERRORLEVEL!==0 exit /b !ERRORLEVEL!
    popd

    echo ***Running CMAKE for ARM***
    if EXIST %cmake-root%\cmake\pnpbridge_arm (
        rmdir /s/q %cmake-root%\cmake\pnpbridge_arm
    )
    mkdir %cmake-root%\cmake\pnpbridge_arm
    rem no error checking

    pushd %cmake-root%\cmake\pnpbridge_arm
    cmake -Drun_longhaul_tests:BOOL=%CMAKE_run_longhaul_tests% -Drun_e2e_tests:BOOL=%CMAKE_run_e2e_tests% -Drun_sfc_tests:BOOL=%CMAKE_run_sfc_tests% -Drun_unittests:BOOL=%CMAKE_run_unittests% %build-root% -Duse_prov_client:BOOL=%prov_auth% -G "Visual Studio 15 ARM" -Duse_edge_modules=%use_edge_modules%
    if not !ERRORLEVEL!==0 exit /b !ERRORLEVEL!
    ...
    cmake -Drun_longhaul_tests:BOOL=%CMAKE_run_longhaul_tests% -Drun_e2e_tests:BOOL=%CMAKE_run_e2e_tests% -Drun_sfc_tests:BOOL=%CMAKE_run_sfc_tests% -Drun_unittests:BOOL=%CMAKE_run_unittests% %build-root% -Duse_prov_client:BOOL=%prov_auth% -G "Visual Studio 15" -A ARM64  -Duse_edge_modules=%use_edge_modules%
    if not !ERRORLEVEL!==0 exit /b !ERRORLEVEL!
) else (
    echo ***Running CMAKE for Win32***
...
if %MAKE_NUGET_PKG% == yes (
    if %make%==yes (
        ...
        msbuild /m %cmake-root%\cmake\iotsdk_win32\azure_iot_pnp_bridge.sln /p:"Configuration=Release;TargetPlatformVersion=%build-winsdkver%"
        if !ERRORLEVEL! neq 0 exit /b !ERRORLEVEL!
        msbuild /m %cmake-root%\cmake\iotsdk_win32\azure_iot_pnp_bridge.sln /p:"Configuration=Debug;TargetPlatformVersion=%build-winsdkver%"
        if not !ERRORLEVEL!==0 exit /b !ERRORLEVEL!

        msbuild /m %cmake-root%\cmake\pnpbridge_x64\azure_iot_pnp_bridge.sln /p:"Configuration=Release;TargetPlatformVersion=%build-winsdkver%"
        if !ERRORLEVEL! neq 0 exit /b !ERRORLEVEL!
        msbuild /m %cmake-root%\cmake\pnpbridge_x64\azure_iot_pnp_bridge.sln /p:"Configuration=Debug;TargetPlatformVersion=%build-winsdkver%"
        if not !ERRORLEVEL!==0 exit /b !ERRORLEVEL!

        msbuild /m %cmake-root%\cmake\pnpbridge_arm\azure_iot_pnp_bridge.sln /p:"Configuration=Release;TargetPlatformVersion=%build-winsdkver%"
        if !ERRORLEVEL! neq 0 exit /b !ERRORLEVEL!
        msbuild /m %cmake-root%\cmake\pnpbridge_arm\azure_iot_pnp_bridge.sln /p:"Configuration=Debug;TargetPlatformVersion=%build-winsdkver%"
        if not !ERRORLEVEL!==0 exit /b !ERRORLEVEL!
    )
) else (
    if %make%==yes (
        msbuild /m azure_iot_pnp_bridge.sln /p:"TargetPlatformVersion=%build-winsdkver%"
        if !ERRORLEVEL! neq 0 exit /b !ERRORLEVEL!
...
:clean-a-solution
call :_run-msbuild "Clean" %1 %2 %3 %4
goto :eof


:build-a-solution
call :_run-msbuild "Build" %1 %2 %3 %4
goto :eof
...
if "%~5" neq "" set build-winsdkver=%~5


msbuild /m %build-target% "/p:Configuration=%build-config%;Platform=%build-platform%;TargetPlatformVersion=%build-winsdkver%" %2
if not !ERRORLEVEL!==0 exit /b !ERRORLEVEL!
goto :eof