...
echo ---------- Setting up the environment for RPi2 ---------- 
...
FILE="$build_root/scripts/linux/toolchain-rpi.cmake" 
...
cd $build_root 
sed -i 's/\[device connection string\]/'$IOTHUB_DEVICE_CONN_STR'/g' deps/azure-iot-sdk-c-pnp/iothub_client/samples/iothub_ll_telemetry_sample/iothub_ll_telemetry_sample.c

# ----------------------------------------------------------------------------- 
# -- Build the SDK 
# -----------------------------------------------------------------------------
echo ---------- Building the SDK by executing build.sh script ---------- 
cd $build_root/scripts/linux 
